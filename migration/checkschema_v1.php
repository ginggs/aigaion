...
*/
function checkDatabaseV1_19($bSilent = false) {
 if (checkVersion("V1.19",$bSilent)) return true;
 if (!checkDatabaseV1_18($bSilent)) return false;

 
 if (!$bSilent)
     echo ("Updating version V1.19... "); //for debug
 
 if (!setVersion("V1.19", $bSilent))  return false;

 setReleaseVersion("1.3.4");
...
- update to release 1.3.3
*/
function checkDatabaseV1_18($bSilent = false) {
 if (checkVersion("V1.18",$bSilent)) return true;
 if (!checkDatabaseV1_17($bSilent)) return false;

 
 if (!$bSilent)
     echo ("Updating version V1.18... "); //for debug
 
 if (!setVersion("V1.18", $bSilent))  return false;

 setReleaseVersion("1.3.3");
...
*/
function checkDatabaseV1_17($bSilent = false) {
 if (checkVersion("V1.17",$bSilent)) return true;
 if (!checkDatabaseV1_16($bSilent)) return false;
 ...
 $Q = mysql_query("SELECT * FROM publication");
 while ($R = mysql_fetch_array($Q))
 {
   $Q2 = mysql_query("UPDATE publication SET actualyear='".getActualYearFromPublicationRow($R)."'
                                              WHERE pub_id='".$R["pub_id"]."'");
 }
 
 if (!setVersion("V1.17", $bSilent))  return false;

 setReleaseVersion("1.3.2");
...
*/
function checkDatabaseV1_16($bSilent = false) {
 if (checkVersion("V1.16",$bSilent)) return true;
 if (!checkDatabaseV1_15($bSilent)) return false;

 include_once("authorfunctions.php");

 if (!$bSilent)
     echo ("Updating version V1.16... "); //for debug
 $res = mysql_query("ALTER TABLE `publication` CHANGE `title` `title` TEXT NOT NULL ");
 ...
 $Q = mysql_query("SELECT pub_id, year, actualyear, title, cleantitle, journal, cleanjournal FROM publication");
 while ($R = mysql_fetch_array($Q))
 {
   $cleantitle = $R['title'];
   ...
   $Q2 = mysql_query("UPDATE publication SET year='".trim(addslashes($R['year']))."',
 ...
 if ($res) {
   while ($row=mysql_fetch_array($res)) {
     mysql_query("UPDATE author set cleanname='".addslashes(getCleanNameFromRow($row))."' WHERE ID=".$row["ID"]);
   }
 }

 if (!setVersion("V1.16", $bSilent))  return false;

 setReleaseVersion("1.3.1");
...
*/
function checkDatabaseV1_15($bSilent = false) {
  if (checkVersion("V1.15",$bSilent)) return true;
  if (!checkDatabaseV1_14($bSilent)) return false;

  if (!$bSilent)
  	echo ("Updating version V1.15... "); //for debug
  $res = mysql_query("ALTER TABLE `config` CHANGE `value` `value` TEXT NOT NULL ");
  ...
  if (!setVersion("V1.15", $bSilent))  return false;
...
*/
function checkDatabaseV1_14($bSilent = false) {
  if (checkVersion("V1.14",$bSilent)) return true;
  if (!checkDatabaseV1_13($bSilent)) return false;
  ...
  $res = mysql_query("CREATE TABLE `availablerights` (`name` varchar(20) NOT NULL, `description` varchar(255) NOT NULL, PRIMARY KEY  (`name`)) TYPE=MyISAM;");
  if (!$res) {
      dbError(mysql_error());
      return false;
  }
  
  $query = "INSERT INTO `availablerights` (`name`, `description`) VALUES ";
  ...
  $res = mysql_query("CREATE TABLE `userrights` (`user_id` INT(10) NOT NULL, `right_name` VARCHAR(20) NOT NULL, PRIMARY KEY (`right_name`,`user_id`)) TYPE=MyISAM;");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
  ...
  if ($res) {
    while ($row = mysql_fetch_array($res)) {
      foreach ($rightmap[$row["u_rights"]] as $right) {
        $res2 = mysql_query("INSERT INTO `userrights` (`user_id`, `right_name`) VALUES ({$row['ID']},'{$right}')");
        if (!$res2) {
          dbError(mysql_error());
          return false;
        }
  ...
  if (!setVersion("V1.14", $bSilent))  return false;
...
*/
function checkDatabaseV1_13($bSilent = false) {
  if (checkVersion("V1.13",$bSilent)) return true;
  if (!checkDatabaseV1_12($bSilent)) return false;

  if (!$bSilent)
  	echo ("Updating version V1.13... "); //for debug

  $res = mysql_query("ALTER TABLE `person` ADD `authordisplaystyle` VARCHAR(5) NOT NULL DEFAULT 'vlf' AFTER `summarystyle`");
  ...
  if (!setVersion("V1.13", $bSilent))  return false;
...
*/
function checkDatabaseV1_12($bSilent = false)
{
	$dbversion = "V1.12";
	if (checkVersion($dbversion, $bSilent)) return true;
	if (!checkDatabaseV1_11($bSilent)) return false;

	if (!$bSilent) echo ("Updating to database version {$dbversion}... "); //for debug

	if (!( mysql_query("ALTER TABLE `author` CHANGE name firstname VARCHAR(255) NOT NULL") )) {
		dbError(mysql_error());
		return false;
	}

	if (!( mysql_query("ALTER TABLE `person` ADD `email` VARCHAR(30) NOT NULL default '' AFTER `abbreviation`") )) {
		dbError(mysql_error());
		return false;
	}

	if (!setVersion($dbversion, $bSilent))  return false;
...
*/
function checkDatabaseV1_11($bSilent = false)
{
	if (checkVersion("V1.11", $bSilent)) return true;
	if (!checkDatabaseV1_10($bSilent)) return false;

	if (!$bSilent)
  	echo ("Updating version V1.11... "); //for debug

  $Q = mysql_query("ALTER TABLE `person` DROP `database`;");
  if (!$Q)
  {
    dbError(mysql_error());
    return false;
  ...
  setReleaseVersion("1.2.1");

  if (!setVersion("V1.11", $bSilent))  return false;
...
*/
function checkDatabaseV1_10($bSilent = false)
{
  include_once("specialcharfunctions.php");

  if (checkVersion("V1.10",$bSilent)) return true;
  if (!checkDatabaseV1_9($bSilent)) return false;

  if (!$bSilent)
  	echo ("Updating version V1.10... "); //for debug

  $Q = mysql_query("ALTER TABLE `personpublicationnote` DROP `xrefs`;");
  if (!$Q)
  {
    dbError(mysql_error());
    return false;
  ...
  $Q = mysql_query("CREATE TABLE `notecrossrefid` (`note_id` INT( 10 ) NOT NULL ,`xref_id` INT( 10 ) NOT NULL);");
  if (!$Q)
  {
    dbError(mysql_error());
    return false;
  ...
  $Q = mysql_query("SELECT pub_id, bibtex_id FROM publication WHERE bibtex_id != ''");
  if (!$Q)
  {
    dbError(mysql_error());
    return false;
  ...
  else
  {
  	while ($R = mysql_fetch_array($Q))
  	{
  		$Q2 = mysql_query("SELECT note_id FROM personpublicationnote WHERE text LIKE '%".addslashes($R['bibtex_id'])."%'");
  		if (mysql_num_rows($Q2) > 0)
  		{
  			while ($R2 = mysql_fetch_array($Q2))
  			{
	  			$Q3 = mysql_query("INSERT INTO notecrossrefid (`note_id`, `xref_id`) VALUES ('".$R2['note_id']."', '".$R['pub_id']."')");
	  			if (!$Q3)
				  {
				  	echo "INSERT INTO notecrossrefid (`note_id`, `xref_id`) VALUES ('".$R2['note_id']."', '".$R['pub_id']."')";
				    dbError(mysql_error());
				    return false;
				  }
				}
  ...
  $Q = mysql_query("SELECT title, journal FROM publication");
  if ($Q)
  {
    while ($R=mysql_fetch_array($Q))
    {
      ...
      mysql_query("UPDATE publication SET cleantitle='".addslashes(stripQuotesFromString($cleantitle))."', cleanjournal='".addslashes(stripQuotesFromString($cleanjournal))."' WHERE pub_id=".$R["pub_id"]);
    }
  }

  if (!setVersion("V1.10", $bSilent))  return false;
...
*/
function checkDatabaseV1_9($bSilent = false) {

  if (checkVersion("V1.9",$bSilent)) return true;
  if (!checkDatabaseV1_8($bSilent)) return false;

  if (!$bSilent)
  	echo ("Updating version V1.9... "); //for debug

  //Add xrefs field for reference speed up.
  $Q = mysql_query("ALTER TABLE `personpublicationnote` CHANGE `xrefs` `xrefs` VARCHAR( 255 ) DEFAULT '' NOT NULL");
  if (!$Q)
  {
    dbError(mysql_error());
    return false;
  ...
  if (!setVersion("V1.9", $bSilent))  return false;
...
*/
function checkDatabaseV1_8($bSilent = false) {
  include_once("publicationfunctions.php");
  include_once("authorfunctions.php");

  if (checkVersion("V1.8",$bSilent)) return true;
  if (!checkDatabaseV1_7($bSilent)) return false;
  ...
  $res = mysql_query("ALTER TABLE `publication` ADD `actualyear` VARCHAR(12) NOT NULL DEFAULT '0000' AFTER `year`");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
  ...
  $Q = mysql_query("ALTER TABLE `publication` CHANGE `title` `title` TEXT NOT NULL ");
  if (!$Q)
  {
    dbError(mysql_error());
    return false;
  ...
  if ($res) {
    while ($row=mysql_fetch_array($res)) {
      ...
      mysql_query("UPDATE publication SET actualyear='".addslashes($actualyear)."' WHERE pub_id=".$row["pub_id"]);
    }
  }


  if (!setVersion("V1.8", $bSilent))  return false;
...
*/
function checkDatabaseV1_7($bSilent = false)
{
  if (checkVersion("V1.7",$bSilent)) return true;
  if (!checkDatabaseV1_6($bSilent)) return false;

  if (!$bSilent)
  	echo ("Updating version V1.7... "); //for debug

  setReleaseVersion("1.2");

  if (!setVersion("V1.7", $bSilent))  return false;
...
*/
function checkDatabaseV1_6($bSilent = false)
{
  if (checkVersion("V1.6",$bSilent)) return true;
  if (!checkDatabaseV1_5($bSilent)) return false;

  if (!$bSilent)
  	echo ("Updating version V1.6... "); //for debug

	//Convert firstpage from int to varchar
  $Q = mysql_query("ALTER TABLE `publication` CHANGE `firstpage` `firstpage` VARCHAR( 10 ) DEFAULT '0' NOT NULL");
  if (!$Q)
  {
    dbError(mysql_error());
    return false;
  ...
  $Q = mysql_query("ALTER TABLE `publication` CHANGE `lastpage` `lastpage` VARCHAR( 10 ) DEFAULT '0' NOT NULL");
  if (!$Q)
  {
    dbError(mysql_error());
    return false;
  ...
  //Convert lastpage from int to varchar
  $Q = mysql_query("ALTER TABLE `publication` CHANGE `chapter` `chapter` VARCHAR( 10 ) NOT NULL DEFAULT '0'");
  if (!$Q)
  {
    dbError(mysql_error());
    return false;
  ...
  //Add xrefs field for reference speed up.
  $Q = mysql_query("ALTER TABLE `personpublicationnote` ADD `xrefs` VARCHAR( 255 ) NOT NULL");
  if (!$Q)
  {
    dbError(mysql_error());
    return false;
  ...
  if (!setVersion("V1.6", $bSilent))  return false;
...
*/
function checkDatabaseV1_5($bSilent = false) {
  include_once("authorfunctions.php");
  include_once("specialcharfunctions.php");
  include_once("publicationfunctions.php");

  if (checkVersion("V1.5",$bSilent)) return true;
  if (!checkDatabaseV1_4($bSilent)) return false;
  ...
  #add column 'von'
  $res = mysql_query("ALTER TABLE `author` ADD `von` VARCHAR(255) NOT NULL default '' AFTER `surname`");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
  ...
  #change year column
  $res = mysql_query("ALTER TABLE `publication` CHANGE `year` `year` VARCHAR(12) NOT NULL DEFAULT '0000'");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
  ...
  $res = mysql_query("ALTER TABLE `author` ADD `cleanname` VARCHAR(255) NOT NULL default '' AFTER `specialchars`");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
  ...
  $res = mysql_query("ALTER TABLE `publication` ADD `cleantitle` VARCHAR(255) NOT NULL default '' AFTER `specialchars`");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
  $res = mysql_query("ALTER TABLE `publication` ADD `cleanjournal` VARCHAR(255) NOT NULL default '' AFTER `specialchars`");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
  ...
  if ($res) {
    while ($row=mysql_fetch_array($res)) {
      ...
      mysql_query("UPDATE publication SET cleantitle='".addslashes(stripQuotesFromString($cleantitle))."', cleanjournal='".addslashes(stripQuotesFromString($cleanjournal))."' WHERE pub_id=".$row["pub_id"]);
    }
  }

  if (!setVersion("V1.5", $bSilent))  return false;
...
*/
function checkDatabaseV1_4($bSilent = false) {
  include_once("PARSEENTRIES.php");
  include_once("doifunctions.php");

  if (checkVersion("V1.4",$bSilent)) return true;
  if (!checkDatabaseV1_3($bSilent)) return false;

  if (!$bSilent)
  	echo ("Updating version V1.4... "); //for debug

  #add column
  $res = mysql_query("ALTER TABLE `publication` ADD `doi` VARCHAR(255) NOT NULL default '' AFTER `url`");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
    ...
    if (trim($userfields)!="") {
	  ...
	  if (isset($entries[0]["doi"])) {
        ...
        if (trim($doi) != "") {
          ...
          $res2 = mysql_query("UPDATE publication SET doi='".addslashes($doi)."' WHERE pub_id=".$row["pub_id"]);
          if (!$res2) {
          dbError(mysql_error());
            return false;
          }
  ...
  if (!setVersion("V1.4", $bSilent))  return false;
