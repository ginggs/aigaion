...
*/
function checkDatabaseV0_14($bSilent = false) {
  if (checkVersion("V0.14",$bSilent)) return true;
  if (!checkDatabaseV0_13($bSilent)) return false;

  if (!$bSilent)
  	echo ("Updating version V0.14... "); //for debug

  //insert public field for preference. Default is 'FALSE'
  $res = mysql_query("ALTER TABLE `publication` ADD `url` VARCHAR(255) NOT NULL default '' AFTER `abstract`");
  ...
  if (!setVersion("V0.14",$bSilent))  return false;
...
*/
function checkDatabaseV0_13($bSilent=false) {
  if (checkVersion("V0.13",$bSilent)) return true;
  if (!checkDatabaseV0_12($bSilent)) return false;
  if (!$bSilent)
  	echo ("Updating version V0.13... "); //for debug

  $res = mysql_query("ALTER TABLE `person` ALTER `theme` SET DEFAULT 'darkdefault'");
  ...
  if ($res) {
    while ($row=mysql_fetch_array($res)) {
        if (findSpecialCharsInArray($row)) {
            $res2 = mysql_query("UPDATE publication SET specialchars='TRUE' WHERE pub_id=".$row["pub_id"]);
            if (!$res2) {
              dbError(mysql_error());
              return false;
            }
  ...
  if ($res) {
    while ($row=mysql_fetch_array($res)) {
        if (findSpecialCharsInArray($row)) {
            $res2 = mysql_query("UPDATE author SET specialchars='TRUE' WHERE ID=".$row["ID"]);
            if (!$res2) {
              dbError(mysql_error());
              return false;
            }
  ...
  if (!setVersion("V0.13", $bSilent))  return false;
...
*/
function checkDatabaseV0_12($bSilent=false) {
  if (checkVersion("V0.12",$bSilent)) return true;
  if (!checkDatabaseV0_11($bSilent)) return false;
  if (!$bSilent)
  	echo ("Updating version V0.12... "); //for debug

  //insert public field for preference. Default is 'FALSE'
  $res = mysql_query("ALTER TABLE `person` ADD `newwindowforatt` enum('TRUE','FALSE') NOT NULL default 'FALSE' AFTER `theme`");
  ...
  if (!setVersion("V0.12",$bSilent))  return false;
...
*/
function checkDatabaseV0_11($bSilent=false) {
  if (checkVersion("V0.11",$bSilent)) return true;
  if (!checkDatabaseV0_10($bSilent)) return false;
  if (!$bSilent)
  	echo ("Updating version V0.11... "); //for debug

  //insert public field for style. Default is 'author'
  $res = mysql_query("ALTER TABLE `person` ADD `summarystyle` VARCHAR( 255 ) NOT NULL DEFAULT 'author' AFTER `theme`");
  ...
  if (!setVersion("V0.11",$bSilent))  return false;
...
*/
function checkDatabaseV0_10($bSilent=false) {
  if (checkVersion("V0.10",$bSilent)) return true;
  if (!checkDatabaseV0_9($bSilent)) return false;
  if (!$bSilent)
  	echo ("Updating version V0.10... "); //for debug

  //insert public field for themes. Default is 'default'
  $res = mysql_query("ALTER TABLE `person` ADD `theme` VARCHAR( 255 ) NOT NULL DEFAULT 'darkdefault' AFTER `ID`");
  ...
  if (!setVersion("V0.10",$bSilent))  return false;
...
function checkDatabaseV0_9($bSilent=false) {
  if (checkVersion("V0.9",$bSilent)) return true;
  if (!checkDatabaseV0_8($bSilent)) return false;
  if (!$bSilent)
  ...
  $data = mysql_query("SELECT pub_id, type FROM publication");

  $alter = mysql_query("ALTER TABLE `publication` CHANGE `type` `type` ENUM( 'Article', 'Book', 'Booklet', 'Inbook', 'Incollection', 'Inproceedings', 'Manual', 'Mastersthesis', 'Misc', 'Phdthesis', 'Proceedings', 'Techreport', 'Unpublished', 'Journal', 'BookSection', 'Report', 'Dissertation' )");
  if (!$alter) {
    dbError(mysql_error());
  ...
  while ($row = mysql_fetch_array($data)) {
    ...
    $result = mysql_query("UPDATE publication SET type='".$newType."' WHERE pub_id=".$row['pub_id']);
  }

  //and add new field: crossref, namekey and userfields
  $res = mysql_query("ALTER TABLE `publication` ADD `crossref` VARCHAR( 255 ) NOT NULL");
  if (!$res) {
    dbError(mysql_error());
  ...
  $res = mysql_query("ALTER TABLE `publication` ADD `namekey` VARCHAR( 255 ) NOT NULL");
  if (!$res) {
    dbError(mysql_error());
  ...
  $res = mysql_query("ALTER TABLE `publication` ADD `userfields` TEXT NOT NULL");
  if (!$res) {
    dbError(mysql_error());
  ...
  $res = mysql_query("ALTER TABLE `publication` ADD `specialchars` ENUM( 'FALSE', 'TRUE' ) NOT NULL DEFAULT 'FALSE'");
  if (!$res) {
    dbError(mysql_error());
  ...
  $res = mysql_query("ALTER TABLE `author` ADD `specialchars` ENUM( 'FALSE', 'TRUE' ) NOT NULL DEFAULT 'FALSE'");
  if (!$res) {
    dbError(mysql_error());
    //echo "Error at action 5";
    return false;
  ...
  $alter = mysql_query("ALTER TABLE `publication` CHANGE `type` `type` ENUM( 'Article', 'Book', 'Booklet', 'Inbook', 'Incollection', 'Inproceedings', 'Manual', 'Mastersthesis', 'Misc', 'Phdthesis', 'Proceedings', 'Techreport', 'Unpublished')");
  if (!$alter) {
    dbError(mysql_error());
  ...
  if (!setVersion("V0.9",$bSilent))  return false;
...
publications
*/
function checkDatabaseV0_8($bSilent=false) {
  if (checkVersion("V0.8",$bSilent)) return true;
  if (!checkDatabaseV0_7($bSilent)) return false;
  if (!$bSilent)
  	echo ("Updating version V0.8... "); //for debug

  //new columns isremote, name & mime for publicationfile
  $res = mysql_query("ALTER TABLE `publicationfile` ADD `isremote` ENUM( 'TRUE', 'FALSE' ) NOT NULL DEFAULT 'FALSE' AFTER `person_id`");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
  $res = mysql_query("ALTER TABLE `publicationfile` ADD `name` VARCHAR(255) NOT NULL DEFAULT '' AFTER `person_id`");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
  $res = mysql_query("ALTER TABLE `publicationfile` ADD `mime` VARCHAR(100) NOT NULL DEFAULT '' AFTER `person_id`");
  if (!$res) {
    dbError(mysql_error());
    return false;
  }
  ...
  $res = mysql_query("SELECT * FROM publicationfile");
  if ($res) {
    while ($row = mysql_fetch_array($res)) {
        ...
        $res2 = mysql_query("UPDATE `publicationfile` SET name='".addslashes($row["location"])."', mime='".addslashes($mime)."' WHERE pub_id=".$row["pub_id"]." AND location='".addslashes($row["location"])."'");
        if (!$res2) {
          dbError(mysql_error());
          return false;
        }
  ...
  //  return false;
  //}
  if (!setVersion("V0.8",$bSilent))  return false;
...
*/
function checkDatabaseV0_7($bSilent=false) {
  if (checkVersion("V0.7",$bSilent)) return true;
  if (!checkDatabaseV0_6($bSilent)) return false;
  if (!$bSilent)
  	echo ("Updating version V0.7... "); //for debug

  //insert public field for notes on a publication. Default is public
  $res = mysql_query("ALTER TABLE `personpublicationnote` ADD `rights` ENUM( 'public', 'private' ) NOT NULL DEFAULT 'public' AFTER `person_id`");
  ...
  //  return false;
  //}
  if (!setVersion("V0.7",$bSilent))  return false;
...
*/
function checkDatabaseV0_6($bSilent=false) {
    if (checkVersion("V0.6",$bSilent))return true;
    if (!checkDatabaseV0_5($bSilent))  return false;
    if (!$bSilent)
    	echo ("Updating version V0.6... "); //for debug

    // Default mark of publication is 5
    $res = mysql_query("ALTER TABLE `publication` CHANGE `mark` `mark` INT( 11 ) DEFAULT '5' NOT NULL");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }
    ...
    $Q1 = mysql_query("SELECT * FROM publication");
    if (!$Q1) {
        dbError(mysql_error());
        return false;
    }
    while ($R1 = mysql_fetch_array($Q1))
    {
        ...
        $Q2 = mysql_query("INSERT IGNORE INTO personpublicationmark (pub_id,person_id,`mark`,`read`) VALUES ($pub_id, $entered_by_person,-1, 'y') ");
        if (!$Q2) {
            dbError(mysql_error());
            return false;
        }
        //next: make sure this entry has its read-attribute on 1
        $Q3 = mysql_query("UPDATE personpublicationmark SET `read`='y' WHERE pub_id=$pub_id AND person_id=$entered_by_person");
        if (!$Q3) {
            dbError(mysql_error());
            return false;
        }
    }
    if (!setVersion("V0.6",$bSilent))  return false;
    return true;
}
...
*/
function checkDatabaseV0_5($bSilent=false) {
    if (checkVersion("V0.5",$bSilent))return true;
    if (!checkDatabaseV0_4($bSilent))  return false;
    if (!$bSilent)
    	echo ("Updating version V0.5... "); //for debug

    // extend bibtex attribute of pub table
    $res = mysql_query("ALTER TABLE publication MODIFY `bibtex_id` VARCHAR( 255 ) NOT NULL;");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }
    ...
    $res = mysql_query("ALTER TABLE author MODIFY `name` VARCHAR( 255 ) NOT NULL;");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }
    ...
    $res = mysql_query("ALTER TABLE author MODIFY `surname` VARCHAR( 255 ) NOT NULL;");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }
    ...
    $res = mysql_query("ALTER TABLE author MODIFY `email` VARCHAR( 255 ) NOT NULL;");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }
    ...
    $res = mysql_query("ALTER TABLE author ADD institute VARCHAR( 255 ) NOT NULL;");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }

    if (!setVersion("V0.5",$bSilent))  return false;
    return true;
}
...
*/
function checkDatabaseV0_4($bSilent=false) {

    if (checkVersion("V0.4",$bSilent))return true;
    if (!checkDatabaseV0_3($bSilent))  return false;
    if (!$bSilent)
    	echo ("Updating version V0.4... "); //for debug

    // add column 'abstract', text, to table 'publication'
    $res = mysql_query("ALTER TABLE publication ADD abstract TEXT NOT NULL;");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }

    if (!setVersion("V0.4",$bSilent))  return false;
    return true;
}
...
Changes in database structure:
    ...
    in persontopic.
*/
function checkDatabaseV0_3($bSilent=false) {

    if (checkVersion("V0.3",$bSilent))return true;
    if (!checkDatabaseV0_2($bSilent)) return false;
    if (!$bSilent)
    ...
    $res = mysql_query("ALTER TABLE person ADD lastreviewedtopic INT(10) NOT NULL default '1';");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }
    ...
    $res = mysql_query("SELECT person_id, MAX(topic_id) FROM persontopic GROUP BY person_id");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }
    while ($row=mysql_fetch_array($res)) {
        $res2 = mysql_query("UPDATE person SET lastreviewedtopic='".$row["MAX(topic_id)"]."' WHERE ID='".$row["person_id"]."'");
        if (!$res2) {
            dbError(mysql_error());
            return false;
        }
    ...
    $res = mysql_query("DELETE FROM persontopic WHERE subscribed!='1';");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }
    ...
    $res = mysql_query("ALTER TABLE persontopic DROP subscribed;");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }

    if (!setVersion("V0.3",$bSilent))  return false;
    return true;
...
*/
function checkDatabaseV0_2($bSilent=false) {
    if (checkVersion("V0.2",$bSilent))return true;
    if (!checkDatabaseV0_1($bSilent)) return false;
    if (!$bSilent)
    ...
    $res = mysql_query("ALTER TABLE persontopic ADD subscribed INT(2) NOT NULL default '1';");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }

    if (!setVersion("V0.2",$bSilent))  return false;
    return true;
}
...
*/
function checkDatabaseV0_1($bSilent=false) {
    if (checkVersion("V0.1", $bSilent))return true;
    if (!checkDatabaseV0($bSilent)) return false;
    if (!$bSilent)
    ...
    $res = mysql_query("CREATE TABLE `aigaiongeneral` (`version` varchar(10) NOT NULL, PRIMARY KEY  (`version`)) TYPE=MyISAM;");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }
    //The versioning field will not yet be initialized, create a row with empty version field
    $res = mysql_query("INSERT INTO `aigaiongeneral` (`version`) VALUES ('')");
    if (!$res) {
        dbError(mysql_error());
        return false;
    }

    if (!setVersion("V0.1",$bSilent))  return false;
    return true;
}
