...
if ( 		$pwd !=''
	...
	if ($res) {
			...
			foreach ($keyword_array as $entry) {
				...
				if (mysql_num_rows($res) > 0) {
					...
					$res = _query("INSERT INTO keywords (keyword) VALUES ('".mysql_real_escape_string($entry['keyword'])."')");
	...
	_query("ALTER TABLE `personpublicationmark`
			...
			DROP PRIMARY KEY,
			ADD PRIMARY KEY  USING BTREE(`pub_id`, `user_id`);");
