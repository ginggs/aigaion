...
if ( $pwd !='' 
    ...
    if ( 
        ...
        $theDatabase = mysql_connect(AIGAION1_DB_HOST,
                                     AIGAION1_DB_USER,
                                     AIGAION1_DB_PWD);
        if ($theDatabase)
        {
        ...
        _query("CREATE TABLE `rightsprofiles` (`rightsprofile_id` INT( 10 ) NOT NULL AUTO_INCREMENT PRIMARY KEY, `name` VARCHAR( 20 ) NOT NULL);");
        ...
        _query("ALTER TABLE `publicationfile` ADD COLUMN `att_id` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `isremote`;");
        _query("ALTER TABLE `publicationfile` MODIFY COLUMN `att_id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT, ADD PRIMARY KEY(`att_id`);");
        ...
        _query("ALTER TABLE `topics` ADD COLUMN `user_id` INTEGER UNSIGNED NOT NULL DEFAULT 0;");
        
        _query("ALTER TABLE `attachments` ADD COLUMN `read_access_level` ENUM('private','public','intern','group') NOT NULL DEFAULT 'intern';");
	    ...
	    if ($res) {
	        while ($row = mysql_fetch_array($res)) {
                ...
                _query('UPDATE author SET '.
                                'surname="'.addslashes(bibCharsToUtf8FromString($row['surname'])).
                                '", von="'.addslashes(bibCharsToUtf8FromString($row['von'])).
                                '", firstname="'.addslashes(bibCharsToUtf8FromString($row['firstname'])).
                                '", institute="'.addslashes(bibCharsToUtf8FromString($row['institute'])).
	    ...
	    if ($res) {
	        while ($row = mysql_fetch_array($res)) {
                ...
                _query('UPDATE publication SET '.
                                '   title="'.addslashes(bibCharsToUtf8FromString($row['title'])).
                                '", series="'.addslashes(bibCharsToUtf8FromString($row['series'])).
                                '", publisher="'.addslashes(bibCharsToUtf8FromString($row['publisher'])).
                                '", location="'.addslashes(bibCharsToUtf8FromString($row['location'])).
                                '", journal="'.addslashes(bibCharsToUtf8FromString($row['journal'])).
                                '", booktitle="'.addslashes(bibCharsToUtf8FromString($row['booktitle'])).
                                '", institution="'.addslashes(bibCharsToUtf8FromString($row['institution'])).
                                '", address="'.addslashes(bibCharsToUtf8FromString($row['address'])).
                                '", organization="'.addslashes(bibCharsToUtf8FromString($row['organization'])).
                                '", school="'.addslashes(bibCharsToUtf8FromString($row['school'])).
                                '", note="'.addslashes(bibCharsToUtf8FromString($row['note'])).
                                '", abstract="'.addslashes(bibCharsToUtf8FromString($row['abstract'])).
