...
class Publications extends Controller {
  ...
  */
  function keyword()
  {
    ...
    //load keyword
    $keywordResult = $this->keyword_db->getByID($keyword_id);
    ...
    //set header data
    $header ['title']       = 'Keyword: "'.$keyword.'"';
    $header ['javascripts'] = array('tree.js','scriptaculous.js','builder.js','prototype.js');
    $header ['sortPrefix']       = 'publications/keyword/'.$keyword_id.'/';
    $header ['exportCommand']    = '';//'export/keyword/'.$keyword_id.'/';
    $header ['exportName']    = 'Export for keyword';

    //set data
    $publicationContent['header']       = 'Publications for keyword "'.$keyword.'"';
    switch ($order) {
        case 'type':
            $publicationContent['header']          = 'Publications for keyword "'.$keyword.'" sorted by journal and type';
            break;
        case 'recent':
            $publicationContent['header']          = 'Publications for keyword "'.$keyword.'" sorted by recency';
            break;
        case 'title':
            $publicationContent['header']          = 'Publications for keyword "'.$keyword.'" sorted by title';
            break;
        case 'author':
            $publicationContent['header']          = 'Publications for keyword "'.$keyword.'" sorted by first author';
            break;
    }
    if ($userlogin->getPreference('liststyle')>0) {
        //set these parameters when you want to get a good multipublication list display
        ...
        $publicationContent['multipageprefix'] = 'publications/keyword/'.$keyword_id.'/'.$order.'/';
    }    
    $publicationContent['publications'] = $this->publication_db->getForKeyword($keyword_id,$order);
