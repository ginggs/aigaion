...
echo sprintf(__("The system detected that you were logged out while submitting a form named '%s'.
      The data in that form has <b>not</b> yet been submitted successfully to the database. 
      Press the button below to re-submit the form data."), $this->latesession->get('FORMREPOST_formname'))."<br/><br/>";
if ($userlogin->isAnonymous()) {
    echo "<div class='errormessage'>";
    echo __("<b>NOTE</b>: You are now logged in as a guest user. If you submitted the form from a 
            registered account, you should first login with your registered account before 
            resubmitting the form, because from this guest account you might then not have enough 
            rights to submit the form.")."<br/><br/>";
    echo "</div>";
}
...
echo form_close();
echo form_open('');
echo form_submit('cancel', __('Cancel form'));
echo form_hidden('form_reposted','form_reposted');
echo form_close();
echo "</div>";
